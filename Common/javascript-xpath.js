 ...
 * class: BinaryExpr
 */
var BinaryExpr = function(op, left, right, datatype) {
    this.op = op;
    this.left = left;
    this.right = right;
 ...
 * NodeType
 */
var NodeType = function(name, literal) {
    this.name = name;
 ...
 */
var VariableReference = function(name) {
...
/*@end @*/
    ...
    substring: [function(s, n1, n2) {
        ...
        return s.substring(n1 - 1, n1 + n2 - 1) 
    }, 'string', false, []],
...
function NodeCount(xPath)
{
	...
	if(result.snapshotLength > 0)
	   {
			parent = result.snapshotItem(0).parentNode;
	   }
	for (var i = 1; i < result.snapshotLength; i++)
        {
		   if( result.snapshotItem(i).parentNode == parent)
			{
			  bSameParent = true;
			  return result.snapshotLength.toString() + "," + bSameParent.toString();
			}
		}
	resultString = result.snapshotLength.toString() + "," + bSameParent.toString();
	return  resultString;
}
var css = document.createElement('style');
css.type = 'text/css';

var styles = '.qualitia_highlight { border-style:solid !important; border-color:#FF0000 !important; border-width:2px !important;} '
			+'.qualitia_mouseover { border-style:solid !important; border-color:#0000FF !important; border-width:2px !important;} '
			+'.bubble{position:relative;background:#f6ff00;padding:15px;color:#000;border-radius: 10px;} '
			+'.bubble:after {content:"";display:block;position:relative;bottom:-30px;left:50px;width:0;height:0;border-width:15px 15px 15px 15px;border-style:solid;border-color: #f6ff00 transparent ;}';
...
var htmlElementString = "<div id='qualitia_label_div' style='position:absolute;z-index:100;display:none; height:25px; top:0; left:0;!important'><span id='qualitia_label_span' style='background:#B0C4DE'></span></div>"
var htmlLabelElement = CreateHtmlElement(htmlElementString);

document.body.insertBefore(htmlLabelElement, null);

var previousElements = new Array();
var currentElements = new Array();
var htmlElement;

function HighlightElements(isHighlight) {
	...
	{	
		...
		{
			htmlElement = currentElements[i]; 
			AddClass(htmlElement, "qualitia_highlight")
			htmlElement.scrollIntoView(false);
			previousElements.push(htmlElement);
		}
...
function RemoveHighlightEffect()
{
	...
	{
		htmlElement = previousElements[i];  
		RemoveClass(htmlElement, "qualitia_highlight")
	}
	
	previousElements = new Array();
}

function LocateElementByXPath(xPath) {
	var result = null;
	RemoveHighlightEffect();
    result = document.evaluate(xPath, document, null, 7, null);
	if(result != null)
	{
		for (var i = 0; i < result.snapshotLength; i++) {
			currentElements.push(result.snapshotItem(i));
		}		
...
function LocateElementByName(name) {
	var elements = null;
