...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="Suite">
    ...
    <div class="container">
       <div id="logodate">
            ...
            <table style="width:100%">
              <tr>
                ...
                <td align='left' colspan="4">
                  <span><xsl:value-of select="$varSuiteName" /></span>
                </td>
              </tr>
              <tr>
                ...
                <td align='left' colspan="4">
                  <span><xsl:value-of select="@ExecutionTime" /></span>
                </td>
              </tr>
              <tr>
				...
				<td class="attrname"> : </td>
                <td align='left'>
                  <span><xsl:value-of select="@StartTime" /></span>
                </td>
                ...
                <td style="margin:0 !important">
					...
					<td align='left'>
					<xsl:choose>
					  <xsl:when test="contains(../@Browser,'GOOGLECHROME')">
              ...
              <tr>
				...
				<td class="attrname"> : </td>
                <td align='left'>
                  <span><xsl:value-of select="@EndTime" /></span>
                </td>
				<td class="attrname">Environment</td>
				<td class="attrname"> : </td>
                <td align='left'>
                  <span><xsl:value-of select="$varExecutionEnvironment"/>
				  </span>
                </td>
          ...
          <span id="date" style="font-size:small;"></span>
       </div>
    </div>
    <div class="clear"></div>
  ...
  <xsl:template match="TCs">
    <xsl:for-each select="TC">
      <xsl:call-template name="TCTemplate"></xsl:call-template>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="TCTemplate">
	...
	<xsl:variable name="varScenarioName">
		<xsl:call-template name="escape-spl-chars">
			<xsl:with-param name="text" select="@ScenarioName"/>
		</xsl:call-template>
	</xsl:variable>
	
    <tr id="{$tcID}">
      <xsl:attribute name="class">
        <xsl:if test="@ExecutionTime='00:00:00.000'">
          disabled_row
        </xsl:if>
      </xsl:attribute>
      ...
      <td><xsl:value-of select="$varScenarioName" /></td>
      <td><xsl:value-of select="@ManualTCId" /></td>
      <td><center>
        <xsl:choose>
  ...
  <xsl:template name="TCIterationTemplate">
    ...
    <xsl:variable name="tcIteration"><xsl:value-of select="@IterationNo" /></xsl:variable>
    
    <tr id="{$tcIterationID}">
      <xsl:attribute name="onclick">
        <xsl:if test="@ExecutionTime!='00:00:00.000'">NavigateToTC('<xsl:value-of select="$strURL"/>','<xsl:value-of select="$tcIteration"/>','<xsl:value-of select="$BLN_INFO_LOG"/>', '<xsl:value-of select="$BLN_DEBUG_LOG"/>', '<xsl:value-of select="$BLN_ERROR_LOG"/>')
        </xsl:if>
      </xsl:attribute>
      <xsl:attribute name="class">
        <xsl:if test="@ExecutionTime='00:00:00.000'">
        ...
        child-of-<xsl:value-of select="$rowId"/>
      </xsl:attribute>
      <td style="border-left-width: 1px;">
        <img src="Images/TCIteration.png"></img>
      ...
      <td>&#160;</td> <!--It is Tc Itertaiton row, no need to display scenarion name. added spaces insted of scenario name-->
      <td>&#160;</td> <!--It is Tc Itertaiton row, no need to display manul tc id. added spaces insted of manul tcid-->
      <td><center>
        <xsl:choose>
          <xsl:when test="@Status=0">
            <img src="Images/passed.png"/>
